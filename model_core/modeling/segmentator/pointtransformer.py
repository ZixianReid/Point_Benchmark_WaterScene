...
class PointTransformer(torch.nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.k = cfg.MODEL.POINT_TRANSFORMER.K
        self.in_channels = cfg.MODEL.POINT_TRANSFORMER.IN_CHANNELS
        self.out_channels = cfg.MODEL.POINT_TRANSFORMER.NUM_CLASSES
        self.dim_model = cfg.MODEL.POINT_TRANSFORMER.DIM_MODEL

        # dummy feature is created if there is none given
        in_channels = max(self.in_channels, 1)

        # first block
        self.mlp_input = MLP([in_channels, self.dim_model[0]], plain_last=False)

        self.transformer_input = TransformerBlock(
            in_channels=self.dim_model[0],
            out_channels=self.dim_model[0],
        )
        ...
        for i in range(0, len(self.dim_model) - 1):
            # Add Transition Down block followed by a Point Transformer block
            self.transition_down.append(
                TransitionDown(in_channels=self.dim_model[i],
                               out_channels=self.dim_model[i + 1], k=self.k))

            self.transformers_down.append(
                TransformerBlock(in_channels=self.dim_model[i + 1],
                                 out_channels=self.dim_model[i + 1]))

            # Add Transition Up block followed by Point Transformer block
            self.transition_up.append(
                TransitionUp(in_channels=self.dim_model[i + 1],
                             out_channels=self.dim_model[i]))

            self.transformers_up.append(
                TransformerBlock(in_channels=self.dim_model[i],
                                 out_channels=self.dim_model[i]))

        # summit layers
        self.mlp_summit = MLP([self.dim_model[-1], self.dim_model[-1]], norm=None,
                              plain_last=False)

        self.transformer_summit = TransformerBlock(
            in_channels=self.dim_model[-1],
            out_channels=self.dim_model[-1],
        )

        # class score computation
        self.mlp_output = MLP([self.dim_model[0], 64, self.out_channels], norm=None)
